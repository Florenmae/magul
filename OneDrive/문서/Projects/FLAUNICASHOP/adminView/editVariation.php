...
<h4>Edit Product Variation Details</h4>
<?php
    include_once "../config/dbconnect.php";
	$ID=$_POST['record'];
	$qry=mysqli_query($conn, "SELECT * from product_details Where variation_id='$ID'");
...
<form id="update-Items" onsubmit="updateVariations()" enctype='multipart/form-data'>
    ...
    <div class="form-group">
        ...
        <?php
            ...
            if ($result-> num_rows > 0){
            while($row = $result-> fetch_assoc()){
                echo"<option selected value='".$row['shop_id']."'>".$row['shop_name'] ."</option>";
            }
            }
        ?>
        <?php
    ...
    <div class="form-group">
        <label for="stockss">Stock Quantity:</label>
        <input type="number" class="form-control" id="stocks" value="<?=$row1['stocks']?>" required>
    </div>
    <div class="form-group">
